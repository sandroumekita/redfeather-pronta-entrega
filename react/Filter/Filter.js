      ...
      <div className={`${style.filtersContainer} ${open ? style.open : " "}`}>
        <div className={style.filters}>
          {filters.length > 0
            ? filters.map((filter) => {
                if (
                  filter.type == "PriceRanges" ||
                  filter.title == "Indicações (teste campo )" ||
                  filter.title == "Marca"
                )
                  return null;
                if (filter.title == "Cor") {
                    ...
                    <div className={style.filterContainer}>
                      ...
                      <div className={style.filterContentColor}>
                        {filter.facets.map((facet) => {
                          return Object.keys(colors).map((i) => {
                              ...
                              if (facet.selected)
                                return (
                                  <a
                                    ...
                                    title={facet.name}
                                    className={style.filterItemColorSelected}
                                    href={`/${facet.href}`}
                                  ></a>
                                );
                              if (colors[i].image)
                                return (
                                  <a
                                    style={{
                                      backgroundImage: `url(${colors[i].value}`,
                                    }}
                                    title={facet.name}
                                    className={style.filterItemColor}
                                    href={`/${facet.href}`}
                                  ></a>
                                );
                              return (
                      ...
                      <div className={style.filterContentTamanho}>
                        {filter.facets.map((facet) => {
                          if (facet.selected)
                            return (
                              <a
                                className={style.filterItemTamanhoSelected}
                                href={`/${facet.href}`}
                              >
                                {facet.name}
                              </a>
                          ...
                          return (
                            <a
                              className={style.filterItemTamanho}
                              href={`/${facet.href}`}
                            >
                              {facet.name}
                            </a>
